...
export let is_results_mode = true;
export let pip_prop = [];
...
async function load(hold, lablArray, d3mRootPath, d3mDataName, d3mPreprocess, d3mData, d3mPS, d3mDS, pURL) {
 ...
 //   set('outputType', d3mOutputType);
    ...
    let len = pipeline_obj.length;

    for(let i = 0 ; i< pipeline_obj.length ; i++){
        for(let j=0 ; j< pipeline_obj[i].modules.length ; j++){
            let obj = {
                id: len++,
                reflexive: false,
                name: pipeline_obj[i].modules[j].id,
                labl: pipeline_obj[i].modules[j].label,
                data: [5, 15, 20, 0, 5, 15, 20],
                count: hold,
                nodeCol: pipeline_obj[i].nodeCol, // Need to change to different shade ; add a function
                baseCol: pipeline_obj[i].nodeCol,
                strokeColor: selVarColor,
                strokeWidth: "1",
                subsetplot: false,
                subsetrange: ["", ""],
                ...
                group2: true,
                forefront: false,
                parent: pipeline_obj[i].pipelineId
...
function result_layout(v, v2) {
    ...
    if (v == layoutAdd || v == layoutMove) {
        ...
        if(IS_D3M_DOMAIN) {
            ...
            nodes = allNodes.slice(0,allNodes.length);    // Add all but first variable on startup (assumes 0 position is d3m index variable)
    ...
    restart = function($links, is_explore) {
        ...
        if (forcetoggle[0] == "true") {
            force.gravity(0.05);
            force.charge(d => setPebbleCharge(d*-5));
            force.start();
        ...
        g.append('svg:circle')
            ...
            .on('dblclick', function(d) {
                ...
                if(d.group1 && d.expand){
                    d.expand = false;
                    for(var p = 0;p < pip_prop.length;p++){
                        if(pip_prop[p].parent == d.name){
                            nodes = nodes.filter(function(node) {
                                return node.id !== pip_prop[p].id;
                            });
                            links = links.filter(function(link){
                                return link.target.id !== pip_prop[p].id;
                            });
                        }
                    }
                    console.log(allNodes)
                    ...
                    for(let i = 0 ; i < pip_prop.length ; i++){
